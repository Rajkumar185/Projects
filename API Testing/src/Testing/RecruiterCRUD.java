...
public class RecruiterCRUD {
	...
	public static void main(String[] args) {
		...
		// Signup
		String SignUp = given().log().all().header("Content-Type", "application/json").body(rd.addSignUpData()).when()
				.post("http://localhost:7171/api/auth/recruiter/signup").then().assertThat().statusCode(200).extract()
				.response().asString();
...
//		RecruiterId = RecruiterIdd;
		setRecruiterId(RecruiterIdd);
		System.out.println(RecruiterId);
		// SignIn
		String SignIn = given().log().all().header("Content-Type", "application/json").body(rd.addSigninData()).when()
				.post("http://localhost:7171/api/auth/recruiter/signin").then().assertThat().statusCode(200).extract()
				.response().asString();
		System.out.println(SignIn);

		// To update the profile of recruiter
		String Updateprofile = given().log().all().header("Content-Type", "application/json").body(rd.updateData())
				.when().put("http://localhost:7673/recruiter/updateprofile").then().assertThat().statusCode(200)
				.extract().response().asString();
		System.out.println(Updateprofile);
		// For getting all the recruiters
		String AllRecruiter = given().log().all().header("Content-Type", "application/json").when()
				.get("http://localhost:7673/recruiter/all").then().assertThat().statusCode(200).extract().response()
				.asString();
		System.out.println(AllRecruiter);
		// For getting a recruiter by his name
		String RecruiterName = given().log().all().header("Content-Type", "application/json").when()
				.get("http://localhost:7673/recruiter/" + rd.getUsername()).then().assertThat().statusCode(200)
				.extract().response().asString();
